...
//					p.setAge(Integer.parseInt(!vals[3].equals("null") ? vals[3] : "0"));
					p.setBirthDate(!vals[3].equals("null") ? LocalDate.parse(vals[3], formatter) : LocalDate.now());
	...
	private LocalDate parse(String input, Locale loc) throws ParseException {
		DateTimeFormatter formatter =
		DateTimeFormatter.ofPattern("dd/MM/yyyy", loc);
		LocalDate d = null;
		try {
		d = LocalDate.parse(input, formatter);
		} catch (DateTimeParseException e){
		throw new ParseException("date not correct",1);
		}
		return d;}
...
//				.append((p.getAge() != 0 ? p.getAge() : null) + ";").append(p.getEmailAddress() + ";")
				.append((p.getBirthDate() != null ? formatter.format(p.getBirthDate()) : null) + ";").append(p.getEmailAddress() + ";")
